...
public:
   ...
   * The face values of the cards range from 2 to 14, with 2 representing a Two and 14 representing an Ace.
   * 
...
public:
  ...
  static bool PlayRound(Deck &player1, Deck &player2, Deck &spoils) {
    ...
    if (p1 < p2) {
      player2.place_bottom(std::move(spoils));
    } else {
      player1.place_bottom(std::move(spoils));
    }
    spoils = Deck{};
